"""
fact_checker.py ÔÇö VERSION RAPIDE + SOURCES COMPL├êTES
 
Optimisations de vitesse :
- Toutes les affirmations sont v├®rifi├®es EN PARALL├êLE (ThreadPoolExecutor)
- Wikipedia FR + EN et Google News sont recherch├®s en parall├¿le pour chaque claim
- R├®sum├®s Wikipedia r├®cup├®r├®s uniquement pour l'article le plus pertinent
- Timeout r├®duit ├á 5s pour ne pas bloquer

Qualit├® :
- Wikipedia et Google News toujours affich├®s dans les sources
- Groq utilise sa connaissance + r├®sum├®s complets Wikipedia
- Prompt optimis├® pour ├®viter les faux n├®gatifs
"""
...
def wiki_search(query: str, lang: str = "fr", max_results: int = 2) -> List[Dict]:
    ...
    try:
        ...
        print(f"  [Wiki {lang.upper()}] {len(results)} r├®sultat(s) pour '{query[:35]}'")
        return results
...
def wiki_summary(title: str, lang: str = "fr") -> str:
    """R├®cup├¿re le r├®sum├® complet d'un article Wikipedia."""
    try:
        ...
        print(f"  [Wiki r├®sum├®] '{title}' ÔåÆ {len(extract)} car.")
        return extract[:1200]
    except Exception:
        return ""
...
class FactChecker:
    ...
    def _groq(self, prompt: str, max_tokens: int = 400) -> str:
        ...
        try:
            r = self.client.chat.completions.create(
                ...
                temperature=0.1,
                messages=[
                    {"role": "system", "content": "Tu es un fact-checker expert. R├®ponds UNIQUEMENT en JSON valide, sans texte avant ni apr├¿s, sans balises markdown."},
    ...
    # ÔöÇÔöÇ Extraction des affirmations ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    def extract_claims(self, text: str) -> List[str]:
        """Extrait jusqu'├á 8 affirmations v├®rifiables depuis le texte."""
        # Limiter ├á 4000 caract├¿res pour la vitesse
        prompt = f"""Extrais les affirmations factuelles v├®rifiables de ce texte (chiffres, dates, statistiques, noms + faits pr├®cis).
...
Maximum 8 affirmations."""
    ...
    # ÔöÇÔöÇ V├®rification d'une affirmation ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    def _fetch_sources(self, claim: str) -> Tuple[List, List, List, List]:
        """
        R├®cup├¿re Wikipedia FR, Wikipedia EN, News EN PARALL├êLE.
        ...
        # R├®sum├®s Wikipedia (en parall├¿le aussi)
        to_summarize = []
        ...
        if to_summarize:
            with ThreadPoolExecutor(max_workers=2) as ex:
                ...
                for f in as_completed(futures):
                    ...
                    s = f.result()
                    if s:
                        summaries.append(f"[Wikipedia {lang.upper()} ÔÇö {title}]\n{s}")

        return wiki_fr, wiki_en, news, summaries

    def verify_claim(self, claim: str) -> Dict[str, Any]:
        """V├®rifie une affirmation. Wikipedia + News toujours affich├®s."""
        print(f"\n[VERIFY] {claim[:60]}")

        wiki_fr, wiki_en, news, summaries = self._fetch_sources(claim)

        # Contexte pour Groq
        wiki_ctx = "\n\n".join(summaries) if summaries else "Aucun r├®sum├® Wikipedia disponible."
        news_ctx = ""
        for n in news:
            news_ctx += f"ÔÇó {n['title']} ÔÇö {n['snippet'][:200]}\n"
        if not news_ctx:
            news_ctx = "Aucun article de presse trouv├®."

        prompt = f"""Tu es un fact-checker rigoureux.

AFFIRMATION : "{claim}"

WIKIPEDIA (r├®sum├®s encyclop├®diques fiables) :
{wiki_ctx}

GOOGLE NEWS (presse r├®cente) :
{news_ctx}

R├êGLES :
1. Utilise EN PRIORIT├ë ta propre connaissance factuelle
2. Les r├®sum├®s Wikipedia sont fiables pour les faits ├®tablis
3. Google News est fiable pour les ├®v├®nements r├®cents
4. Si Wikipedia et News se contredisent ÔåÆ pr├®f├¿re ta connaissance + News pour les faits r├®cents
5. Un snippet court ou hors contexte NE suffit PAS ├á invalider un fait connu
6. Ne marque JAMAIS "Faux" un fait que tu sais ├¬tre vrai

JSON sur une seule ligne :
{{"score": 85, "verdict": "V├®rifi├®", "explanation": "Explication pr├®cise en 1-2 phrases."}}

Bar├¿me : 85-100=confirm├®, 65-84=probable, 45-64=incertain, 25-44=douteux, 0-24=faux
Verdict : exactement un de ÔåÆ V├®rifi├®, Probable, Ind├®termin├®, Trompeur, Faux"""
        ...
        valid = ["V├®rifi├®", "Probable", "Ind├®termin├®", "Trompeur", "Faux"]
        if verdict not in valid:
            if score >= 75: verdict = "V├®rifi├®"
            elif score >= 55: verdict = "Probable"
            elif score >= 35: verdict = "Ind├®termin├®"
            elif score >= 20: verdict = "Trompeur"
            else: verdict = "Faux"

        # ÔöÇÔöÇ Construire les sources affich├®es ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        if not any("News" in s["source"] for s in display):
            q = requests.utils.quote(claim[:60])
            display.append({
                "title": f"Rechercher dans l'actualit├® : {claim[:45]}...",
                "url": f"https://news.google.com/search?q={q}&hl=fr",
                "source": "Google News",
            })

        print(f"  ÔåÆ {score}% | {verdict} | sources: {[s['source'] for s in display]}")
        return {"text": claim, "score": score, "verdict": verdict, "explanation": expl, "sources": display}

    # ÔöÇÔöÇ Analyse compl├¿te ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    def calculate_global_score(self, results: List[Dict]) -> Dict:
        if not results:
            return {"score": 50, "verdict": "Ind├®termin├®", "color": "amber", "summary": "Aucune affirmation analys├®e."}
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        """
        Analyse compl├¿te ÔÇö les affirmations sont v├®rifi├®es EN PARALL├êLE
        pour r├®duire le temps d'analyse au maximum.
        """
        import time
        t0 = time.time()
        print(f"\n{'='*55}\n[ANALYSE] {len(text)} caract├¿res\n{'='*55}")

        claims = self.extract_claims(text)
        if not claims:
            return {
                "score": 50, "verdict": "Ind├®termin├®", "color": "amber",
                "summary": "Aucune affirmation factuelle v├®rifiable d├®tect├®e. Essayez un texte avec des chiffres, dates ou noms pr├®cis.",
                "claims": [],
            }

        # ÔöÇÔöÇ V├ëRIFICATION PARALL├êLE de toutes les affirmations ÔöÇÔöÇ
        # Chaque affirmation est v├®rifi├®e dans son propre thread
        # ÔåÆ Temps total Ôëê temps d'une seule v├®rification (au lieu de N ├ù T)
        ...
        with ThreadPoolExecutor(max_workers=min(len(claims), 6)) as ex:
            ...
            for future in as_completed(future_to_idx):
                ...
                try:
                    ...
                    results[idx] = {
                        "text": claims[idx], "score": 50, "verdict": "Ind├®termin├®",
        ...
        print(f"\n[R├ëSULTAT] {global_result['score']}% ÔÇö {global_result['verdict']} ({elapsed:.1f}s)")
